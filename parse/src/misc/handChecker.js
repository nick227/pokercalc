...
function evalHand (cards) {
  ...
  var val = null; var id = null; var suit = null; var cardsObj = []; var result = []; var count = 0
...
function matchCheck (cards, type) {
  var num = 1; var tmp = []; var tmpO = {}; var tmp = []
...
function compare (needle, obj, type) {
  var tmp = []; var res = null
  var hits = 0; var pair = null
...
var handChecks = [{ name: 'High Card'/* 0 */,
  val: 1,
  fn: function (matches, cardsObj) {
  ...
  tmp: [],
  fn: function (matches, cardsObj) {
				 								var res = { has: false, res: null }; var tmp = []; var obj = {}
  ...
  val: 4,
  fn: function (matches, cardsObj) {
				 								var res = { has: false, res: null }; var tmpO = {}
				 								if (matches.num === 3) {
								 					var vals = getGrouping(matches)
								 					if (vals[0] === 3) {
				 										res = { has: true, res: matches.tmp }
  ...
  val: 5,
  fn: function (matches, cardsObj) {
    var res = { has: false, res: 3 }; var prev = null; var hits = 0; var tmp = []; var counter = 0; var nextCard = null
  ...
  val: 6,
  fn: function (matches, cardsObj) {
				 								var matches = matchCheck(cardsObj, 'suit'); var tmpO = {}; var tmp = []
				 								var res = { has: false, res: null }
				 								if (matches.num > 4) {
  ...
  fn: function (matches, cardsObj) {
    var res = { has: false, res: null }
    var c = null; var tmp = []; var match = null
    var obj = {}
			 				if (matches.num === 4) {
			 					for (var i = 0, length1 = matches.tmp.length; i < length1; i++) {
			 						var match = matches.tmp[i]
        ...
        var res = { has: true, res: matches.tmp }
				 ...
				 { name: 'Straight Flush'/* 8 */,
  val: 8,
  fn: function (matches, cardsObj) {
				 	var res = { has: false, res: null }
				 	var sc = handChecks[4].fn(matches, cardsObj)

    var fc = handChecks[5].fn(matches, cardsObj)
				 	if (sc.has && fc.has) {
